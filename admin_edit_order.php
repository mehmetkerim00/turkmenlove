...
try {
    $stmt = $conn->prepare("UPDATE orders SET client_name = :client_name, client_email = :client_email, client_phone = :client_phone, reciever_name = :reciever_name, reciever_phone = :reciever_phone, city = :city, address = :address, apartment = :apartment, gate = :gate, floor = :floor, comment = :comment, delivery_date = :delivery_date, delivery_time = :delivery_time, quantity = :quantity, product_id = :product_id, no_address = :no_address WHERE id = :id");
    $stmt->execute([
    ...
    $_SESSION['admin_success'] = 'Заказ успешно обновлён';
    header('Location: admin.php');
    exit;
} catch (PDOException $e) {
    error_log("Edit Order Error: " . $e->getMessage());
    $_SESSION['admin_error'] = ['Ошибка при обновлении заказа'];
    header('Location: admin.php');
    exit;
}
?>